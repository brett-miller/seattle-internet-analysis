...
for ipRange in ipRanges:
    rangeStart=int(ipRange['startIpNum']);
    rangeEnd=int(ipRange['endIpNum']);
    rangeSize=rangeEnd-rangeStart;
    rangeStep=max([int(round(rangeSize/10)),1]);
    for ipRangeIndex in range(rangeStart,rangeEnd+1,rangeStep):
        ipList.append({"ip": int(ipRange['startIpNum'])+ipRangeIndex, "ipRange": ipRange})

traceroutes=[]

for ipRange in ipRanges:
    if ipRange["location"]["postalCode"] == "98101":
        print ipRange
